...
import { getUserEscapeTime, getQuizAverageScores, getEscapeRoomRecords } from '../api/record';
...
const Mypage = () => {
  ...
  const doughnutData = averageTime ? {
    ...
    datasets: [{
      ...
      data: [averageTime.roomNumber1, averageTime.roomNumber2, averageTime.roomNumber3, averageTime.roomNumber4], // API로부터 받은 데이터 사용
    ...
    <>
      ...
      <div className="mypage-container">
        ...
        <div className="profile-section">
          <img src={profileImg} alt="Profile" className="profile-picture" /> {/* 하드코딩된 프로필 사진 */}
          <h2>{nickname}</h2> {/* 닉네임 표시 */}
          <button onClick={refreshData} className="refresh-button">갱신</button> {/* 갱신 버튼 */}
        </div>
          ...
          <>
            <div className="stats-container">
              <div className="chart-section">
                <h2>평균 방탈출 시간</h2>
                {doughnutData ? <Doughnut data={doughnutData} /> : <p>데이터가 없습니다.</p>}
              </div>
              ...
              escapeRecords.map((record, index) => (
                <div className="card" key={index}>
                  ...
                  {record.rooms.map((room, roomIndex) => (
                    <p key={roomIndex}>{room.roomName}: {formatTime(room.escapeTime)}</p>
                  ))}
                </div>
              ))
            ) : (
